...
namespace BezierSolution
{
  ...
  public enum SplineType { Normal = 0, Platform = 1, Wall = 2 };
    ...
    public Vector3 GetTangent( float normalizedT )
		...
		{
      if (Utils.FP.IsEqual(startNormalizedT, endNormalizedT, Mathf.Epsilon))
      {
        return 0;
      }
    ...
    //public Vector3 FindNearestPointToOptimised( Vector3 worldPos, out float normalizedT, float accuracy = 100f )
    //{
    //  GetPoint()
    //}
    ...
    public Vector3 FindNearestPointOnXZPlane( Vector3 worldPos, out float normalizedT, float accuracy = 100f, float accuracyWithin = 0.001f )
    {
      ...
      while (!Utils.FP.IsEqual(worldPos.x, result.x, accuracyWithin) || !Utils.FP.IsEqual(worldPos.z, result.z, accuracyWithin))
      {
        ++numSteps;
