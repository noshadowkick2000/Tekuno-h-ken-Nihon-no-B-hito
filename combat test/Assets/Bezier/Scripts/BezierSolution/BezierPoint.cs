...
#pragma warning disable 0649
     ...
     * 
     * UNCOMMENT [FormerlySerializedAs("m_linkedPoint")] BEFORE CHANGING the variable name, and REPLACE "m_linkedPoint" with the CURRENT NAME of the variable!!!
     * 
    ...
    //[FormerlySerializedAs("m_linkedPoint")] <---- NOTICE THIS!
    [SerializeField]
    [HideInInspector]
    private BezierPoint m_linkedPoint = null; // If you want to change this variable name, READ THE ABOVE TEXT!!!!!!!!
    public BezierPoint linkedPoint
    {
      get
      {
        return m_linkedPoint;
      }

      set
      {
        // Setting linked point to nothing
        if (value == null)
        {
          // Check if the linked point still exists
          if (m_linkedPoint)
          {
            // Store the linked point in a temp value since we need to set it as null first
            BezierPoint tempPoint = m_linkedPoint;
            m_linkedPoint = value;

            // Set the stored linked point's value to null as well
            tempPoint.linkedPoint = value;
          }
        }

        // Setting linked point to another point
        else
        {
          // Check if the point already is linked
          if (m_linkedPoint)
          {
            // Since it is switching to another linked point, the existing linked point's linked point should be set to null
            m_linkedPoint.linkedPoint = null;
          }

          m_linkedPoint = value;
        }

        // This sets both linked points to be each other as long as they are not yet
        if (m_linkedPoint != null && m_linkedPoint.linkedPoint != this)
        {
          m_linkedPoint.linkedPoint = this;

          // Copies all parameters 
          cameraBehaviour.CopyFrom(m_linkedPoint.cameraBehaviour);
          position = m_linkedPoint.position;
          precedingControlPointLocalPosition = m_linkedPoint.precedingControlPointLocalPosition;
          followingControlPointLocalPosition = m_linkedPoint.followingControlPointLocalPosition;
          handleMode = m_linkedPoint.handleMode;
        }
      }
    }
    ...
    public CameraBehaviour cameraBehaviour
    {
      get
      {
        m_cameraBehaviour = GetComponent<CameraBehaviour>();

        return m_cameraBehaviour;
      }

      set
      {
        m_cameraBehaviour = value;

        if (m_linkedPoint && m_linkedPoint.cameraBehaviour != cameraBehaviour)
        {
          m_linkedPoint.cameraBehaviour = value;
        }
      }
    }
    ...
    [SerializeField]
        ...
        if (m_linkedPoint && m_linkedPoint.position != position)
        {
          m_linkedPoint.position = value;
        }
			}
		}
#pragma warning restore 0649
        ...
        if (m_linkedPoint && m_linkedPoint.localRotation != localRotation)
        {
          m_linkedPoint.localRotation = value;
        }
      }
		}
        ...
        if (m_linkedPoint && m_linkedPoint.rotation != rotation)
        {
          m_linkedPoint.rotation = value;
        }
      }
		}
        ...
        if (m_linkedPoint && m_linkedPoint.localEulerAngles != localEulerAngles)
        {
          m_linkedPoint.localEulerAngles = value;
        }
      }
		}
        ...
        if (m_linkedPoint && m_linkedPoint.eulerAngles != eulerAngles)
        {
          m_linkedPoint.eulerAngles = value;
        }
      }
		}
        ...
        if (m_linkedPoint && m_linkedPoint.localScale != localScale)
        {
          m_linkedPoint.localScale = value;
        }
      }
		}
...
#pragma warning disable 0649
        ...
        if (m_linkedPoint && m_linkedPoint.precedingControlPointLocalPosition != precedingControlPointLocalPosition)
        {
          m_linkedPoint.precedingControlPointLocalPosition = value;
        }
        ...
        if (m_linkedPoint && m_linkedPoint.precedingControlPointPosition != precedingControlPointPosition)
        {
          m_linkedPoint.precedingControlPointPosition = value;
        }
        ...
        if (m_linkedPoint && m_linkedPoint.followingControlPointLocalPosition != followingControlPointLocalPosition)
        {
          m_linkedPoint.followingControlPointLocalPosition = value;
        }
        ...
        if (m_linkedPoint && m_linkedPoint.followingControlPointPosition != followingControlPointPosition)
        {
          m_linkedPoint.followingControlPointPosition = value;
        }
        ...
        if (m_linkedPoint && m_linkedPoint.handleMode != handleMode)
        {
          m_linkedPoint.handleMode = value;
        }
